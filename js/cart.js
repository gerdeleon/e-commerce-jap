...
function crearProductoHTML(item) {
    ...
    return `
        <div class="producto-en-carrito">
            ...
            <button id="btnEliminar" class="btn btn-eliminar" data-product-id="${id}" style="background-color: rgb(255, 165, 0);">
...
/// FIN DESAFIATEE ///
    ...
    ////////////////////////////////////////////////
// Agrega el evento "click" al botón con ID "btnEliminar"
document.getElementById('btnEliminar').addEventListener('click', function () {
    // Obtén el ID del producto a eliminar del atributo "data-product-id"
  ...
  function eliminarProducto(productId) {
    ...
    let itemDelLocal = lsCarritoData.find((item) => item.id === productId);
  
    if (itemDelLocal) {
      if (itemDelLocal.soldCount > 1) {
        // Si hay más de un artículo del mismo tipo en el carrito, decrementa la cantidad.
        const prod = {
          ...itemDelLocal,
          soldCount: itemDelLocal.soldCount - 1
        };
        let filteredCart = lsCarritoData.filter((item) => item.id !== productId);
        localStorage.removeItem('carrito');
        localStorage.setItem('carrito', JSON.stringify([...filteredCart, prod]));
      } else {
        // Si solo hay un artículo del tipo en el carrito, elimínalo completamente.
        let filteredCart = lsCarritoData.filter((item) => item.id !== productId);
        localStorage.removeItem('carrito');
        localStorage.setItem('carrito', JSON.stringify(filteredCart));
      }
  
      // Llama a la función para actualizar la interfaz después de eliminar el producto.
      actualizarUI(filteredCart);
    }
  }
